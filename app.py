...
with tab2:
    ...
    c_ra = st.checkbox("🚩 8. リスクアセスメント・KY（危険予知）活動の実施（作業前の危険性評価）") # ←★追加
    ...
    if c_river:
        ...
        with st.container():
            ...
            st.markdown("""
            * **【救命措置の義務】** 水中に転落して溺れるおそれのある作業では、浮袋その他の救命具（ライフジャケット等）を備え、救命のための舟を配置する等の措置を講じてください（安衛則 第532条）。
            * **【ボート乗船時の義務】** 小型船舶（ボート等）に乗船して作業を行う場合は、関係法令に基づき、**原則すべての乗船者にライフジャケット（救命胴衣）の着用が義務付けられています**。
            """)

    # ★リスクアセスメントの判定ロジックを追加しました！
    if c_ra:
        any_selected = True
        with st.container():
            st.error("### 📊 リスクアセスメント・KY活動の実施基準（安衛法 第28条の2、第57条の3）")
            st.markdown("""
            * **【一般的な作業・設備（努力義務）】** 建設物、設備、作業行動等に起因する危険性又は有害性の調査（リスクアセスメント）を行い、労働者の危険又は健康障害を防止するため必要な措置を講ずるよう努めてください（安衛法 第28条の2）。地質調査・点検現場は日々環境が変わるため、毎作業前のKY（危険予知）活動の徹底が推奨されます。
            * **【化学物質の取り扱い（法的義務）】** 対象となる化学物質等（SDS交付義務対象物など）を製造・取り扱う場合は、法令に基づくリスクアセスメントの実施が**義務付けられています**（安衛法 第57条の3）。
            """)
...
with tab3:
    ...
    shortcut_cols = st.columns(8) # 8列に変更
    
    # ボタンに「リスクアセスメント」を追加しました！
