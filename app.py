...
with tab2:
    ...
    c_high = st.checkbox("🚩 1. 高所作業（法面、2m以上の足場、昇隔、ロープ高所作業など）")
    ...
    c_river = st.checkbox("🚩 7. 河川・水際・水上での作業（ボート使用、川沿いでの調査・点検など）") # ←★追加
    ...
    if c_river:
        ...
        with st.container():
            ...
            * **【現場環境の確認】** 河川の増水、流速の変化、天候の急変（上流での豪雨など）に対する避難基準を事前に定め、作業員に周知してください。
            """)
...
with tab3:
    ...
    shortcut_cols = st.columns(7) # 7列に変更
    
    # ボタンに「救命胴衣」を追加しました！
    keywords_buttons = ["ボーリング", "ロープ高所作業", "熱中症", "高所作業車", "移動式クレーン", "酸素欠乏", "救命胴衣"]
