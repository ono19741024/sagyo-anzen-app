...
with tab2:
    ...
    c_aerial = st.checkbox("🚩 5. 高所作業車の使用") # ←★ここに追加しました！
    ...
    if c_aerial:
        ...
        with st.container():
            ...
            st.markdown("""
            * **【運転資格】** 作業床の高さが**10m以上**の場合は**『高所作業車運転技能講習修了証』**が、**10m未満**の場合は**『特別教育』**の受講が必要です。
            * **【要求性能墜落制止用器具の使用】** 作業床上の労働者には、必ず**墜落制止用器具（フルハーネス等）**を使用させてください。
            * **【転落・逸走防止】** アウトリガーを張り出すこと。運転者が運転位置から離れるときは、作業床を最低降下位置に置き、ブレーキを確実にかけてください。
            * **【搭乗制限と作業計画】** 乗車席及び作業床以外の箇所に乗せないこと。事前に作業計画を定め、関係労働者に周知してください。
            """)
...
with tab3:
    ...
    # ボタンにも「高所作業車」を追加しました
    keywords_buttons = ["ボーリング", "ロープ高所作業", "高所作業車", "移動式クレーン", "玉掛け", "酸素欠乏"]
